...
This document is the fully populated worked example for AGT-002. It follows the same template as AGT-001 and is the second reference entry for `data/threats.json` and section 5 of the practitioner guide.
...
An AI agent operates with an authorization scope that is decoupled from the authorization scope of the user it is serving. The agent's privileges, identity, and access to systems are configured at deployment time and persist across all user interactions. When a user asks the agent to act, the agent uses its own authorization context, not a constrained version derived from the user's. This creates a structural mismatch in which the agent can take actions the user is not authorized to perform, can access data the user is not authorized to see, or can act on behalf of users it is not serving in the current interaction. The threat is structural, not behavioral: it exists by design in many agent deployments and creates risk even when the agent is functioning correctly.
...
A workflow automation agent in a regulated DACH financial services enterprise is deployed to assist case officers across multiple business units. The agent is configured with read access to the full case management system, write access to specific case fields, and the ability to query an integrated KYC service. This authorization is granted at the system level and is constant across all user interactions, because configuring per-user authorization at the agent layer was deemed operationally complex during deployment.
...
User-context propagation and on-behalf-of models reduce but do not eliminate the threat. Several residual risks remain even with full implementation of recommended controls:
...
| Source | Status |
|---|---|
| Public 2025 to 2026 disclosures of agent authorization issues in enterprise SaaS deployments | Several relevant cases reported by security researchers; specific citations to be selected |
| Academic literature on confused deputy in OAuth and agent contexts | Established literature; useful for theoretical grounding but less compelling as practitioner example |
| Anonymized scenarios from public regulatory enforcement actions | GDPR enforcement decisions involving cross-user data exposure may serve as proxy examples |
...
|---|---|
| AGT-001 (Prompt injection via tool outputs) | Common attack vector that exploits AGT-002; AGT-001 is the input mechanism, AGT-002 is the structural condition |
| AGT-003 (Tool-chain abuse) | Related; tool-chain abuse often depends on the agent having broader authorization than the user, which is AGT-002 |
| AGT-005 (Audit and provenance failure) | Closely related; one consequence of AGT-002 is that audit trails do not cleanly attribute actions to originating users |
| AGT-007 (Inter-agent trust and delegation abuse) | The delegation-ambiguity sub-pattern within AGT-002 connects directly to AGT-007 when the delegated entity is another agent rather than a tool |
...
| Emphasis | Why |
|---|---|
| The threat is structural, not behavioral | CISOs are familiar with detecting bad behavior; they are less prepared for the case where everything is working correctly and harm still occurs |
| Traditional access control is not the answer | Spending more on RBAC or IAM does not address this threat; the architectural pattern must change |
| Audit and attribution problems compound the operational difficulty | Even if a CISO accepts the risk, the inability to cleanly attribute agent actions to users complicates incident response, regulatory reporting, and accountability |
| The three sub-patterns share the same root cause | Treating them as one threat with three facets is more useful than three separate threats; the controls largely overlap |
| Human approval at action boundaries is the fallback | When the controls cannot fully address the threat, this is the honest answer and worth stating directly |
